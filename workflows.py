...
async def _wait_task(task_id: str, poll_seconds: int, allow_gone: bool = False):
    while True:
        state = await workflow.execute_activity(
            check_task, {"task_id": task_id, "allow_gone": allow_gone}, start_to_close_timeout=A_TIMEOUT)
        if state["completed"]:
            return state
        await workflow.sleep(timedelta(seconds=poll_seconds))

async def _run_reindex(command, dry_run, poll_seconds):
    ...
    result = await _wait_task(started["task"], poll_seconds)
    return await workflow.execute_activity(validate_reindex, {"command": command, "task_result": result},
                                           start_to_close_timeout=A_TIMEOUT)

async def _run_enrich(policy, dry_run, poll_seconds):
    ...
    result = {"name": policy["name"], "status": "DRY_RUN" if dry_run else "SUCCESS"}
    if not dry_run:
        task_id = started.get("task")
        if task_id:
            state = await _wait_task(task_id, poll_seconds, allow_gone=True)
            if state.get("gone"):
                result["task_gone"] = True
...
class VsecaElasticReportingWorkflow:
    ...
    async def run(self, inp: dict):
        ...
        poll_seconds = int(inp.get("poll_seconds", 15))

        # Stage 1 and enrich refresh are independent, so execute them in parallel.
        staging_future = asyncio.create_task(_bounded(plan["backend-to-staging"], lambda c: _run_reindex(c, dry_run, poll_seconds), limit))
        enrich_future = asyncio.create_task(_bounded(plan["enrich"], lambda p: _run_enrich(p, dry_run, poll_seconds), limit))
        ...
        reporting = await _bounded(plan["staging-to-reporting"], lambda c: _run_reindex(c, dry_run, poll_seconds), 2)

        # Stage 3 begins only after all Stage 2 commands succeed.
        history = await _bounded(plan["reporting-to-history"], lambda c: _run_reindex(c, dry_run, poll_seconds), 2)
